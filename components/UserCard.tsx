...
const UserCard = ({ name, phone, description, address, image, status }) => {
  return (
    <div className="border rounded-md hover:bg-gray-50 relative">
      <small className="bg-red-500 px-2 py-1 rounded-lg absolute top-2 right-2 text-white">
        {status}
      </small>

      <small className={`px-2 py-1 rounded-lg absolute top-2 right-2 text-white ${
          status === "missing" ? "bg-yellow-500" :
          status === "found" ? "bg-green-500" :
          status === "deceased" ? "bg-red-500" :
